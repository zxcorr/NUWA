...
#include <sys/shm.h>/*. Depending on platform/build flags, this can be missing or behave differently. If you don’t need SysV shared memory headers here, it’s safer to remove it from the public header (or move it into the .c file where it’s actually required). If you added it for parallel.h, I’d still prefer moving it to the implementation file.*/
#include <sys/stat.h> 
/* AM: [start] */
/*#include "errno.h"*/
#include <errno.h>
...
struct transfer {
  ...
  //@{
  /* AM: [start] CMB comoving-distance window shift */
  short has_cmb_chi_window_shift;   /**< master switch */
  double cmb_chi_shift_epsilon;     /**< fractional amplitude of chi shift (|eps| << 1) */
  double cmb_chi_shift_zcut;        /**< sigmoid centre redshift */
  double cmb_chi_shift_dz;          /**< sigmoid width in redshift */

  /* Precomputed fac[it] = 1 + eps * W(z(tau_it)) on ppt->tau_sampling[0..tau_size-1].
   * Populated ONCE in transfer_init() before the parallel q-loop, then memcpy'd into
   * each workspace by transfer_fill_cmb_shift_fac().  NULL when feature is off. */
  double *cmb_shift_fac_precomp;   /**< size = cmb_shift_fac_size */
  int     cmb_shift_fac_size;      /**< = ppt->tau_size when active, else 0 */
  /* AM: [end] */
  ...
  int index_tt_nc_g4;   /**< index for first bin of transfer type = gravity term G3 for of number count */ /* AM: G4 */
  int index_tt_nc_g5;   /**< index for first bin of transfer type = gravity term G3 for of number count */ /* AM: G5 */
...
struct transfer_workspace {
  ...
  /* AM: [start] */
  double * cmb_shift_fac; /* [index_tau] = 1 + eps * W(z(tau)) */
  /* AM: [end] */
