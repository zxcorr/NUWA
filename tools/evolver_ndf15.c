  ...
  */
  ...
  if ((jac->use_sparse)&&(jac->repeated_pattern >= jac->trust_sparse)){
    ...
    for(j=1;j<=neq;j++){
      Fdiff_new = 0.0;
      Fdiff_absrm = 0.0;
      // FBA+AM: FIX 1: Initialize Rowmax[j] to 1 to prevent Segfault if all math fails */
      nj_ws->Rowmax[j] = 1;        
      //
      for(i=1;i<=neq;i++){
      ...
      // FBA+AM:  FIX 2 (Optional but Recommended): Check if we actually found a valid max */
      if (isnan(nj_ws->Difmax[j])) {
      // You might want to print a warning here if your logging system allows
        fprintf(stderr, "Warning: NaN detected in numjac col %d\n", j);
      }        
