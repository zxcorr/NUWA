...
int background_at_tau(
  ...
  class_call(background_z_of_tau(pba,tau,&z),
             pba->error_message,
             pba->error_message);
  /* AM: [start] avoiding spline overshoot*/
  if (z < 0.) z = 0.;
  /* AM: [end] */
  ...
  */
  ...
  /* AM: [start] */    
   if (pba->has_scf == _TRUE_) {
    ...
    class_test(scf_lambda == 0.0,
                 pba->error_message,
                 "scf_lambda = 0 leads to division by zero in attractor IC");
    
    if (pba->attractor_ic_scf == _TRUE_) {
      double denom = 3.0*scf_lambda*scf_lambda - 12.0;
    
      if (denom <= 0.0) {
          /* No attractor for this lambda (would make log argument <= 0) */
        pvecback_integration[pba->index_bi_phi_scf] = 1.0/scf_lambda;
        ...
        if (pba->background_verbose > 0) {
          ...
          double arg = rho_rad * 4.0 / denom;
    
          class_test(arg <= 0.0 || !isfinite(arg),
                     pba->error_message,
                     "Invalid attractor log argument: arg=%e (rho_rad=%e, denom=%e, lambda=%e)",
                     arg, rho_rad, denom, scf_lambda);
    
          pvecback_integration[pba->index_bi_phi_scf] =
            -(1.0/scf_lambda) * log(arg) * pba->phi_ini_scf;
        }
    
        /* Now phi is defined; check potential before sqrt */
        {
          double Vini = V_scf(pba, pvecback_integration[pba->index_bi_phi_scf]);
    
          class_test(!isfinite(Vini) || (Vini < 0.0),
                     pba->error_message,
                     "Invalid V_scf at initial phi: V=%e, phi=%e",
                     Vini, pvecback_integration[pba->index_bi_phi_scf]);
    
          pvecback_integration[pba->index_bi_phi_prime_scf] =
            2.0*a*sqrt(Vini) * pba->phi_prime_ini_scf;
        }
      }
      else {
