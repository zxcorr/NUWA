 ...
 *
 * Modified by A Marins 2026 - CMB Mirage
 * 
 */
...
//SG -- adding self-interaction
    ...
    if (flag1 == _TRUE_){
        ...
        /* AM: [start] */
        if ((flag1 == _TRUE_) &&
            ((strstr(string1,"massive") != NULL) || (strstr(string1,"Massive") != NULL))) {
          pba->has_nuself_massive = _TRUE_;
        }
        else if ((flag1 == _TRUE_) &&
                 ((strstr(string1,"massless") != NULL) || (strstr(string1,"Massless") != NULL))) {
          pba->has_nuself_massless = _TRUE_;
        }
        else {
          class_stop(errmsg,
  ...
  if (ppt->has_cls == _TRUE_) {
    if (ppt->has_scalars == _TRUE_) {
      ...
      if ((ppt->has_cl_lensing_potential == _TRUE_) || (ppt->has_cl_number_count == _TRUE_)){
        ...
        /* AM: [start] */  
      class_read_flag  ("has_cmb_chi_window_shift", ptr->has_cmb_chi_window_shift);
      ppt->has_cmb_chi_window_shift = ptr->has_cmb_chi_window_shift;
      
      if (ptr->has_cmb_chi_window_shift == _TRUE_) {
        class_read_double("cmb_chi_shift_epsilon",    ptr->cmb_chi_shift_epsilon);
        class_read_double("cmb_chi_shift_zcut",       ptr->cmb_chi_shift_zcut);
        class_read_double("cmb_chi_shift_dz",         ptr->cmb_chi_shift_dz);

        ppt->cmb_chi_shift_epsilon    = ptr->cmb_chi_shift_epsilon;
        ppt->cmb_chi_shift_zcut       = ptr->cmb_chi_shift_zcut;
        ppt->cmb_chi_shift_dz         = ptr->cmb_chi_shift_dz;          
        class_test(ptr->cmb_chi_shift_dz <= 0.0,
                   ptr->error_message,
                   "cmb_chi_shift_dz must be > 0");
         /* to keep in 'small shift' regime */
        class_test(fabs(ptr->cmb_chi_shift_epsilon) > 0.2,
                   ptr->error_message,
                    "cmb_chi_shift_epsilon seems too large for a 'small shift' approximation");      
     } /* AM: [end] */  
...
int input_default_params(struct background *pba,
  ...
  /* AM: [start] shift comovel radial distance at early universe? using a atan window function to constrain to the early */  
  ptr->has_cmb_chi_window_shift = _FALSE_;
  ptr->cmb_chi_shift_epsilon = 0.0;
  ptr->cmb_chi_shift_zcut = 50.0;
  ptr->cmb_chi_shift_dz = 10.0;    

  ppt->has_cmb_chi_window_shift = _FALSE_;
  ppt->cmb_chi_shift_epsilon = 0.0;
  ppt->cmb_chi_shift_zcut = 50.0;
  ppt->cmb_chi_shift_dz = 10.0;    
  /* AM: [end]  */
