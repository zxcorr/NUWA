 ...
 */
/* AM: [start] */
static inline double window_B_local(double z, double zcut, double dz){
  return 1.0 / (1.0 + exp(-(z - zcut) / dz)); //0.5 * (1.0 + tanh((z - zcut)/dz));
}
/* AM: [end] */
  ...
  if (ppt->has_scalars == _TRUE_) {
    ...
    if (ppt->has_cls == _TRUE_) {
      ...
      /* choose a k_max_cmb[ppt->index_md_scalars] corresponding to a wavelength on the last
         ...
         k_max_cl[ppt->index_md_scalars]*[comoving.ang.diameter.distance] > l_max */
         /* AM: [start] */

        double chi_shift_rescale = 1.0;
        
        if ((ppt->has_cls == _TRUE_) && (ppt->has_cmb_chi_window_shift == _TRUE_)) {
          double z_rec, W_rec;
          ...
          class_call(background_z_of_tau(pba, pth->tau_rec, &z_rec),
                     pba->error_message,
                     ppt->error_message);
        
          if (z_rec < 0.0) z_rec = 0.0;
        
          W_rec = window_B_local(z_rec,
                                 ppt->cmb_chi_shift_zcut,
                                 ppt->cmb_chi_shift_dz);
        
          chi_shift_rescale = 1.0 + ppt->cmb_chi_shift_epsilon * W_rec;
        
          class_test(chi_shift_rescale <= 0.0,
                     ppt->error_message,
                     "Effective chi_shift_rescale=%g <= 0",
                     chi_shift_rescale);
        }
        
        k_max_cmb[ppt->index_md_scalars] =
          ppr->k_max_tau0_over_l_max * ppt->l_scalar_max
          / pba->conformal_age / pth->angular_rescaling / chi_shift_rescale;
    /* AM: [end] */
      //k_max_cmb[ppt->index_md_scalars] = ppr->k_max_tau0_over_l_max*ppt->l_scalar_max //AM
        // /pba->conformal_age/pth->angular_rescaling;//AM
