...
/* AM: [start] */
#include "background.h"  /* for background_z_of_tau() */
#include <math.h>        /* for exp() */
#include <string.h>      /* for memcpy() */

static inline double window_B(double z, double zcut, double dz){
  /* Logistic sigmoid: W -> 1 at z >> zcut (early universe), W -> 0 at z << zcut */
  return 1.0 / (1.0 + exp(-(z - zcut) / dz));
}

/* Fill ptw->cmb_shift_fac for the current transfer type.
   For primary CMB types (T0/T1/T2/E/B), transfer_sources() uses the
   perturbation tau grid ppt->tau_sampling without redefining the source,
   so the precomputed cache can be copied directly. For all other types,
   fill with unity. */
static int transfer_fill_cmb_shift_fac(
  ...
  short apply =
    (index_tt == ptr->index_tt_t0) ||
    (index_tt == ptr->index_tt_t1) ||
    (index_tt == ptr->index_tt_t2) ||
    (index_tt == ptr->index_tt_e ) ||
    (index_tt == ptr->index_tt_b );

  if ((ptr->has_cmb_chi_window_shift == _FALSE_) || (apply == _FALSE_)) {
    for (index_tau = 0; index_tau < ptw->tau_size; index_tau++)
      ptw->cmb_shift_fac[index_tau] = 1.0;
    return _SUCCESS_;
  }

  /* Fast path: grids match — use precomputed cache (one memcpy). */
  if (ptw->tau_size == ptr->cmb_shift_fac_size) {
    class_test(ptr->cmb_shift_fac_precomp == NULL,
               ptr->error_message,
               "cmb_shift_fac_precomp is NULL although has_cmb_chi_window_shift=TRUE");      
    memcpy(ptw->cmb_shift_fac, ptr->cmb_shift_fac_precomp,
           (size_t)ptw->tau_size * sizeof(double));
    return _SUCCESS_;
  }

  /* AM: Fallback: tau grid size changed (e.g. future CLASS precision change).
   * Recompute per-element from ptw->tau0_minus_tau, which transfer_sources() already
   * filled for this type.  Slower (O(tau_size) spline calls) but correct.
   * A warning is printed so the user knows this path was taken. */
  static int warned_once = 0;
  if (warned_once == 0) {
    fprintf(stderr,
              "WARNING transfer_fill_cmb_shift_fac: size mismatch "
              "(ptw->tau_size=%d, precomp_size=%d) for index_tt=%d. "
              "Falling back to per-element recomputation.\n",
              ptw->tau_size, ptr->cmb_shift_fac_size, index_tt);
    warned_once = 1;
  }    

  for (index_tau = 0; index_tau < ptw->tau_size; index_tau++) {
    double tau = tau0 - ptw->tau0_minus_tau[index_tau];
    double z;
    class_call(background_z_of_tau(pba, tau, &z),
               pba->error_message, ptr->error_message);
    if (z < 0.0) z = 0.0;
    ptw->cmb_shift_fac[index_tau] = 1.0
      + ptr->cmb_chi_shift_epsilon
      * window_B(z, ptr->cmb_chi_shift_zcut, ptr->cmb_chi_shift_dz);
  }
  /* AM: [end] */
...
int transfer_init(
  ...
  */
  ...
  /* AM: [start] Correct angular_rescaling for the chi shift.
   *
   * angular_rescaling drives the l-grid and q-grid spacing (see transfer_get_l_list,
   * transfer_get_q_list, transfer_can_be_neglected).  With the shift active, the
   * effective comoving distance to recombination is chi_rec * fac_rec, where
   * fac_rec = 1 + eps * W(z_rec).  Because W(z_rec=1100) ~ 1 for any reasonable
   * (zcut, dz), fac_rec ~ 1 + eps.  Multiplying angular_rescaling by fac_rec keeps
   * the l and q grids correctly aligned with the shifted acoustic-peak positions.
   *
   * NOTE: This must happen BEFORE transfer_indices() (which reads angular_rescaling
   * to build l and q lists) and BEFORE q_period is computed from it below.
   */
   double z_rec_for_shift = 0.0;
   double W_rec = 1.0;
   double cmb_sampling_rescale = 1.0;
  ...
  /** - correspondence between k and l depend on angular diameter
   ...
   /* For the primary-CMB chi shift, update the transfer sampling scale
      using the same shift evaluated near recombination. */
   if (ptr->has_cmb_chi_window_shift == _TRUE_) {
      class_test(ptr->cmb_chi_shift_dz <= 0.0,
                 ptr->error_message,
                 "cmb_chi_shift_dz must be > 0, got %g",
                 ptr->cmb_chi_shift_dz);
       /* AM: Early flat-universe guard — must be checked before angular_rescaling
        * is modified and before transfer_indices() is called, so that a curved-universe
        * run aborts cleanly before any state has been changed. */
       class_test(pba->sgnK != 0,
                  ptr->error_message,
                  "cmb_chi_window_shift is only supported for flat universes "
                  "(sgnK=0, Omega_k=0). You have sgnK=%d. "
                  "Set Omega_k=0 or disable has_cmb_chi_window_shift.",
                  pba->sgnK);

       class_call(background_z_of_tau(pba, tau_rec, &z_rec_for_shift),
                   pba->error_message,
                   ptr->error_message);
       if (z_rec_for_shift < 0.0) z_rec_for_shift = 0.0;
       
       W_rec = window_B(z_rec_for_shift,
                     ptr->cmb_chi_shift_zcut,
                     ptr->cmb_chi_shift_dz);
       cmb_sampling_rescale = 1.0 + ptr->cmb_chi_shift_epsilon * W_rec;
       class_test(cmb_sampling_rescale <= 0.0,
               ptr->error_message,
               "Effective primary-CMB sampling rescale = %g <= 0. "
               "Choose a less negative cmb_chi_shift_epsilon",
               cmb_sampling_rescale);
       ptr->angular_rescaling *= cmb_sampling_rescale;
    }
  ...
  /* AM: [start] inclusion of 'xmax_rescale': extend Bessel table to cover shifted chi arguments */
  double xmax_rescale = 1.0;
  if (ptr->has_cmb_chi_window_shift == _TRUE_)
    xmax_rescale += fabs(ptr->cmb_chi_shift_epsilon);
  xmax = ptr->q[ptr->q_size-1] * tau0 * xmax_rescale;
  /* AM: [end] */
  if (pba->sgnK == -1)
  ...
  /* AM: [start] Precompute cmb_shift_fac on ppt->tau_sampling ONCE (serial, before threads).
   *
   * For CMB types (T0/T1/T2/E/B), transfer_sources() sets redefine_source=FALSE and fills
   * tau0_minus_tau directly from ppt->tau_sampling — the same grid for every q.  We exploit
   * this to evaluate 1 + eps*W(z(tau)) once here and memcpy the result per-workspace in
   * the hot loop, replacing O(q_size * tau_size) background spline calls with one pass of
   * O(tau_size) spline calls + O(q_size * tau_size) memcpy operations.
   *
   * ppt->tau_size is typically 200–400; q_size is typically 1000–4000.
   * Savings: ~99.9% of the background_z_of_tau overhead on long MCMC chains.
   */
  ptr->cmb_shift_fac_precomp = NULL;
  ptr->cmb_shift_fac_size    = 0;

  if (ptr->has_cmb_chi_window_shift == _TRUE_) {

    /* AM: Flat-universe guard.
     * The chi-shift scales the Bessel argument by (1 + eps*W), which is equivalent
     * to a fractional comoving-distance rescaling only in a flat universe (sgnK=0).
     * In curved space the distance is a nonlinear function of chi (sin or sinh),
     * so the same multiplication does not map to a physical rescaling.
     * Refuse here rather than silently produce wrong C_ells. */
    class_test(pba->sgnK != 0,
               ptr->error_message,
               "cmb_chi_window_shift is only supported for flat universes "
               "(sgnK=0, Omega_k=0). You have sgnK=%d. "
    ...
    const double eps  = ptr->cmb_chi_shift_epsilon;
    const double zcut = ptr->cmb_chi_shift_zcut;
    const double dz   = ptr->cmb_chi_shift_dz;
    const int    ntau = ppt->tau_size;

    class_alloc(ptr->cmb_shift_fac_precomp, ntau * sizeof(double), ptr->error_message);
    ptr->cmb_shift_fac_size = ntau;

    /* Validate dz once before the loop — it is a fixed parameter. */
    class_test(ptr->cmb_chi_shift_dz <= 0.0,
               ptr->error_message,
               "cmb_chi_shift_dz must be > 0, got %g",
               ptr->cmb_chi_shift_dz);
    ...
    for (itau = 0; itau < ntau; itau++) {
      double tau = ppt->tau_sampling[itau];
      double z;
      class_call(background_z_of_tau(pba, tau, &z),
                 pba->error_message, ptr->error_message);
      if (z < 0.0) z = 0.0;   /* guard against floating-point overshoot at tau0 */      
        
      double W = window_B(z, zcut, dz);
      ptr->cmb_shift_fac_precomp[itau] = 1.0 + eps * W;
    }
  }
  /* AM: [end] */
...
int transfer_free(
  ...
  if (ptr->has_cls == _TRUE_) {
    ...
    /* AM: [start] free precomputed chi-shift cache */
    if (ptr->cmb_shift_fac_precomp != NULL) {
      free(ptr->cmb_shift_fac_precomp);
      ptr->cmb_shift_fac_precomp = NULL;
    }
    /* AM: [end] */
  }
      ...
      for (index_ic = 0; index_ic < ppt->ic_size[index_md]; index_ic++) {
        ...
        for (index_tt = 0; index_tt < ptr->tt_size[index_md]; index_tt++) {
          ...
          if ((use_full_limber == _FALSE_) || (index_tt == ptr->index_tt_lcmb)) {
            ...
            /* AM: [start] copy precomputed fac[] into this workspace (O(tau_size) memcpy) */
            class_call(transfer_fill_cmb_shift_fac(pba, ptr, ptw, index_tt,
                                                   pba->conformal_age),
                       ptr->error_message,
                       ptr->error_message);
            /* AM: [end] */
...
int transfer_radial_coordinates(
  ...
  case 1:
    ...
    for (index_tau=0; index_tau < ptw->tau_size; index_tau++) {
      ...
      double fac = (ptw->cmb_shift_fac != NULL) ? ptw->cmb_shift_fac[index_tau] : 1.0;
      class_test(fac <= 0.0, ptr->error_message,
                 "cmb_shift_fac[%d]=%g <= 0; choose a less negative cmb_chi_shift_epsilon",
                 index_tau, fac);

      double chi_unshifted = sqrt_absK * ptw->tau0_minus_tau[index_tau];
      ptw->chi[index_tau] = chi_unshifted * fac;
  ...
  case 0:
    for (index_tau=0; index_tau < ptw->tau_size; index_tau++) {
      ...
      double chi_unshifted = ptr->k[index_md][index_q] * ptw->tau0_minus_tau[index_tau];
      ptw->chi[index_tau] = chi_unshifted * fac;
...
int transfer_update_HIS(
  ...
  if ((ptw->sgnK!=0) && (index_q < ptr->index_q_flat_approximation)) {
    ...
    /*AM: [start] inclusion of 'xmax_rescale': extend curved Bessel table to cover shifted chi arguments */
    double xmax_rescale = 1.0;
    if (ptr->has_cmb_chi_window_shift == _TRUE_)
      xmax_rescale += fabs(ptr->cmb_chi_shift_epsilon);
    xmax = sqrt_absK * tau0 * xmax_rescale;
    /*AM: [end]*/
