  ...
  //if (pba->has_ncdm == _TRUE_) {
      // if ( pba->has_nuself_massive == _TRUE_) {
        //SG - init the interpolation table read by neutrino self-interaction        
         ...
         //}
  /* AM: [start] Collision Table was loading unconditionally <--- pay attention here */    
  if ((pba->has_ncdm == _TRUE_) && (pba->has_nuself_massive == _TRUE_)) {
      //SG - init the interpolation table read by neutrino self-interaction  
      read_and_store_collision_int_coeff_massive(ppr,ppt);
      //class_alloc(ppt->coeff_interp, ppt->l_size*sizeof(double), ppt->error_message); //AM: Stop allocating ppt->coeff_interp globally
  }
  /* AM: [end]*/    
...
/* AM: [start] Collision table was being released unconditionally */
if ((pba->has_ncdm == _TRUE_) && (pba->has_nuself_massive == _TRUE_))
    free_collision_int_coeff_massive(ppt);    
/* AM: [end] */
...
int perturbations_workspace_init(
  ...
  /* AM: [start] */
  ppw->coeff_interp = NULL;

  if ((pba->has_ncdm == _TRUE_) && (pba->has_nuself_massive == _TRUE_)) {
      class_alloc(ppw->coeff_interp,ppt->l_size * sizeof(double),ppt->error_message);  
      
      class_test(ppr->l_max_ncdm < ppt->l_size,
                 ppt->error_message,
                 "l_max_ncdm (%d) must be < ppt->l_size (%d) set by ",
                 "the collision coefficient file.",
                 ppr->l_max_ncdm, ppt->l_size);
...
int perturbations_workspace_free (
  ...
  /* AM: [start] */
  if (ppw->coeff_interp != NULL) {
    free(ppw->coeff_interp);
  }
  /* AM: [end] */    
  return _SUCCESS_;
}
...
int perturbations_find_approximation_switches(
  ...
  //if ((_scalars_) &&
    //  (pba->has_ncdm == _TRUE_) &&
      //(pba->has_nuself_massive == _TRUE_)) {
    //class_alloc(ppw->coeff_interp,
      //          ppt->l_size * sizeof(double),
        //        ppt->error_message);
  //}
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      /** - ----> second case: use exact equation (Boltzmann hierarchy on momentum grid) */
        ...
        /* AM: [start] interaction rate assigned regardless of has_nuself_massive*/
        tau_dot_nu = 0.0; // default: no self-interactions
        if (pba->has_nuself_massive == _TRUE_)
            tau_dot_nu = pvecback[pba->index_bg_dmu_nuself_massive];            
        /* AM: [end] */            
        ...
        //     }
          ...
          for (index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
            ...
            idx += (pv->l_max_ncdm[n_ncdm]+1);
             ...
             if (pba->has_nuself_massive == _TRUE_) {
              class_call(get_collision_int_coeff_massive(ppt,q,ppw->coeff_interp),
                         ppt->error_message,
                         error_message);
            }
    ...
    /** - -----> ncdm shear for given momentum bin */
            ...
            if ( pba->has_nuself_massive == _TRUE_) dy[idx+2] = qk_div_epsilon/5.0*(2*s_l[2]*y[idx+1]-3.*s_l[3]*y[idx+3])
              -s_l[2]*metric_shear*2./15.*dlnf0_dlnq - tau_dot_nu*ppw->coeff_interp[2]*y[idx+2]; //AM: ppt->coedd_interp ---> ppw->coedd_interp
    ...
    /** - -----> ncdm l>3 for given momentum bin */
            ...
            if ( pba->has_nuself_massive == _TRUE_) {
             ...
             for (l=3; l<pv->l_max_ncdm[n_ncdm]; l++){
              dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]) - tau_dot_nu*ppw->coeff_interp[l]*y[idx+l]; //AM: ppt->coedd_interp ---> ppw->coedd_interp
             }    
            }
            if ( pba->has_nuself_massive == _FALSE_) {
    ...
    /** - -----> ncdm lmax for given momentum bin (truncation as in Ma and Bertschinger)
           ...
           if ( pba->has_nuself_massive == _TRUE_)  dy[idx+l] = qk_div_epsilon*y[idx+l-1]-(1.+l)*k*cotKgen*y[idx+l] - tau_dot_nu*ppw->coeff_interp[l]*y[idx+l]; //AM: ppt->coedd_interp ---> ppw->coedd_interp
          ...
          idx += (pv->l_max_ncdm[n_ncdm]+1);
            ...
            //   -s_l[2]*metric_shear*2./15.*dlnf0_dlnq - tau_dot_nu*ppw->coeff_interp[2]*y[idx+2];

            // /** - -----> ncdm l>3 for given momentum bin */

            // for (l=3; l<pv->l_max_ncdm[n_ncdm]; l++){
            //   dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]) - tau_dot_nu*ppw->coeff_interp[l]*y[idx+l];
            //                 // if (abs(q-7.085810e+00)<0.1) { 
            ...
            // dy[idx+l] = qk_div_epsilon*y[idx+l-1]-(1.+l)*k*cotKgen*y[idx+l] - tau_dot_nu*ppw->coeff_interp[l]*y[idx+l];
            ...
            // for (l=3; l<pv->l_max_ncdm[n_ncdm]; l++){
            //   //dy[idx+l] = 0; //qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]) - tau_dot_nu*ppw->coeff_interp[l]*y[idx+l];
            // }

            // /** - -----> ncdm lmax for given momentum bin (truncation as in Ma and Bertschinger)
            //     but with curvature taken into account a la arXiv:1305.3261 */

            // dy[idx+l] = 0;//qk_div_epsilon*y[idx+l-1]-(1.+l)*k*cotKgen*y[idx+l] - tau_dot_nu*ppw->coeff_interp[l]*y[idx+l];
 ...
 /* go through each line */
  while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
    ...
    if (left[0] > 39) {
      ...
      class_test(ppr->l_max_ncdm < ppt->l_size,
                 ppt->error_message,
                 "l_max_ncdm=%d is less than the number of multipoles in the ",
                 "collision file (%d). Raise l_max_ncdm or use a file with ",
                 "fewer multipoles.",
                 ppr->l_max_ncdm, ppt->l_size);          
      /* AM: [end] */
      class_alloc(ppt->x1,ppt->x1_size*sizeof(double),ppt->error_message);
...
int free_collision_int_coeff_massive(struct perturbations * ppt)
{
    ...
    free(ppt->coeff);
    free(ppt->ddcoeff);
    //free(ppt->coeff_interp);    //AM: Stop freeing ppt->coeff_interp in free_collision_int_coeff_massive()
    return _SUCCESS_;
}
